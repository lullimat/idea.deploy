...
- ~~Does pymetallic expose synchronization fine-grained enough to replace `_sync_tenet()`?~~ **Resolved: yes.** No `MTLEvent`/`MTLFence`/completion handlers, but per-command-buffer `get_status()` + `wait_until_completed()`, plus offset-based blit copies, are sufficient — track in-flight buffers and their touched ranges, wait only on overlaps. **Phase 1 is therefore not a prerequisite of Phase 2b.** Details in `docs/residency-probes.md` §2b.
- Still open: does `pymetallic` expose enough of `MTLDevice`/`MTLHeap` to attach an **I/O queue** (`MTLIOCommandQueue`), or does the shim need to own device creation too? This is the Phase 3 storage half, unaffected by the above.
...
- ~~Does the lattice residency case have a natural *eviction* story, or only a streaming one?~~ **Resolved: yes, via halos.** Computing block *b* of a stencil needs *b-1*, *b*, *b+1* and the next step needs *b*, *b+1*, *b+2*, so two of every three acquires hit. The traffic is exactly predictable (`hits = 2(n-1)`, `misses = n+2`), which makes the eviction policy checkable to the integer rather than merely observable. See `docs/residency-probes.md` §2f.
